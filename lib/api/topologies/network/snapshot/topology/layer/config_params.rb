    ...
    class ConfigParams < Grape::API
      ...
      get 'config_params' do
        ...
        nodes.map do |node|
          {
            name: node.name,
            agent_name: ns_converter.node_name_table.find_l1_alias(node.name)['l1_agent'],
            type: node.attribute&.node_type,
            if_list: node.termination_points.map do |tp|
              {
                name: tp.name,
                agent_name: ns_converter.tp_name_table.find_l1_alias(node.name, tp.name)['l1_agent'],
                ipv4: tp.attribute.empty? ? nil : tp.attribute&.ip_addrs&.[](0),
                description: tp.attribute.empty? ? nil : tp.attribute&.description,
                original_if: ns_converter.tp_name_table.reverse_lookup(node.name, tp.name)[1]
              }
            end
          }
        end
