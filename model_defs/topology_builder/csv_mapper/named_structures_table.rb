...
module TopologyBuilder
  module CSVMapper
    ...
    class NamedStructureTableRecord < TableRecordBase
      # @!attribute [rw] node
      #   @return [String]
    ...
    class NamedStructureTable < TableBase
      # @param [String] target Target network (config) data name
      def initialize(target)
        super(target, 'named_structures.csv')
        @records = @orig_table.map { |r| NamedStructureTableRecord.new(r) }
      end

      # @param [String] node_name Node name
      # @param [String] structure_name Structure name
      # @return [nil, NamedStructureTableRecord] Record if found or nil if not found
